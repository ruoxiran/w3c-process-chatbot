...
- Concrete three-layer answer shape for procedural questions. Take "how do I do wide review?" as an example: the answer pattern that makes a step USEFUL combines all three layers — rule from Process, operation from Guidebook, tool to execute. Sketch:
    1. RULE (Process [Sn]): "Process requires Wide Review before CR transition [S1]."
    2. OPERATION (Guidebook [Sm]): "The Guidebook ``documentreview`` chapter says wide review should be requested early — name the audiences, request before stable [S2]."
    3. TOOL (markdown link to action surface): "File the request at [the i18n review tracker](https://github.com/w3c/i18n-request/issues/new/choose) for internationalization review, or at the equivalent ``a11y-request`` / ``privacy-request`` / ``security-request`` tracker for other axes."
