...
- Action surfaces are curated W3C operational addresses, NOT citation excerpts. When you use one, the URL / mailto / repo string itself IS the proof; do not attach a ``[Sn]`` tag to the part of the sentence that names the action surface. Reserve ``[Sn]`` for the substantive claims that come from the cited excerpts. Example: "File at github.com/w3c/i18n-request/issues/new. The W3C Process requires horizontal review before transition [S2]." — the URL has no tag; the rule about horizontal review has [S2].
